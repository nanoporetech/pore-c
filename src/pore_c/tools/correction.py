...
class HiCMap(object):
    ...
    def sinkhorn_knopp_correction(self, max_iter=1000, epsilon=10.0 ** -6):
        """
        ...
        iterations = 0
        while (
            np.any(np.sum(self.cP, axis=1) < e_min)
            or np.any(np.sum(self.cP, axis=1) > e_max)
            or np.any(np.sum(self.cP, axis=0) < e_min)
            or np.any(np.sum(self.cP, axis=0) > e_max)
        ):
...
def compute_contact_probabilities(
    ...
    ci: Optional[float] = 0.999,
    mask_zeros: Optional[bool] = True,
    epsilon: Optional[float] = 10 ** -3,
    max_iter: Optional[float] = 1000,
    eigen: Optional[bool] = False,
) -> None:
